          ...
          return (
            ...
            >
              ...
              {rows.length > 0 ? (
                <ChartFrame
                  title={spec.title}
                  chartId={`trade-${spec.id}`}
                  rows={rows}
                  series={spec.series.map((series, index) => ({ key: series.indicatorId, label: series.label, unit: spec.unit, color: COLORS[index % COLORS.length], yAxisId: series.axis === "right" ? "right" : "left" }))}
                >
                  <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart data={rows} margin={{ top: 24, right: 12, bottom: 4, left: 0 }}>
                      ...
                      <LatestValueLabels rows={rows} series={spec.series.map((series, index) => ({ key: series.indicatorId, label: series.label, unit: spec.unit, color: COLORS[index % COLORS.length], yAxisId: series.axis === "right" ? "right" : "left" }))} />
                    </ComposedChart>
                  </ResponsiveContainer>
                </ChartFrame>
              ) : (
