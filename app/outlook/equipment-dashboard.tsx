        ...
        {CHARTS.map((spec) => {
          ...
          return (
            ...
            >
              ...
              {rows.length > 0 ? (
                <ChartFrame title={spec.title} chartId={`equipment-${spec.id}`} rows={rows} series={spec.series.map((series, index) => ({ key: series.indicatorId, label: series.label, unit: spec.unit, color: COLORS[index % COLORS.length] }))}>
