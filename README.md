...
- **항목명 색인** (`data/ecos-item-names.json`, `scripts/build-ecos-item-index.mjs`) — 표 이름에 없고 **항목 이름에만** 있는 계열에 닿는 경로. ECOS에서 국고채가 든 표의 이름은 "시장금리(일별)"이라, 색인이 없으면 "국고채 커브"는 0건이었다. 런타임 생성은 ECOS 3분 300회 한도를 넘기므로 빌드 산출물로 커밋한다(608표, 약 780KB)
