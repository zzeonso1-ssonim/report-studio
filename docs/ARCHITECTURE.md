...
- **조용한 실패에는 창구를 만든다**: 항목명 색인 부재·ECOS 항목조회 실패·KOSIS 축약 재검색은
  전부 응답(`errors`·`notes`·`indexStatus`)과 `/api/health`로 드러난다. 로그만 남기면 아무도 못 본다.
  색인 로더는 실패를 캐시하지 않고 30초 뒤 재시도한다 — 첫 실패로 인스턴스 수명 내내 꺼지지 않게.
- **능력과 상한은 한 상수에서**: KOSIS 분류 차수는 `KOSIS_OBJ_LEVELS` 하나가 정하고
  어댑터(보낼 수 있는 objL 수)와 검색(결과에 올릴 표의 깊이)이 함께 파생한다.
- **오래 걸리는 동기 작업은 배포 전에 빠르게 만든다**: `/api/chat`은 요청 예산(20초)을 두고
  라운드마다 남은 시간을 확인한다. 429 백오프도 이 예산 안에서만 기다린다 — 서버리스 상한(25초)에
  잘려 5xx가 나가는 대신 우리가 먼저 멈추고 안내를 돌려준다.
...
- **쓴 만큼은 보이게 한다**: `/api/chat` 요청 하나가 태운 라운드 수·OpenAI 호출 수·모델별 토큰
  (캐시 적중분 포함)·소요 시간을 `lib/llm-usage.ts`가 세어 서버 로그 한 줄(`[usage] chat …`)과
  응답 `usage`에 싣는다. **관측만 하고 동작은 바꾸지 않는다** — 라운드 상한을 걸지 말지는
  품질 트레이드오프라 사람이 정할 일이고, 그 판단에 필요한 것은 먼저 숫자다.
  질의 원문·프롬프트·키는 로그에 남기지 않는다.
- **검증은 기본이 무과금이다**: 회귀 스크립트가 매 실행마다 실제 OpenAI를 부르면 개발 루프가
  그대로 청구서가 된다(2026-08-05). 그래서 기본 실행은 픽스처 재생(네트워크 0회)이고 실호출은
  `--live`에서만 나가며, 통과한 실행만 픽스처를 갱신한다. 검색 보조 모델도 개발 경로에서는
  `gpt-4o-mini`로 고정해 환경변수 재정의를 받지 않는다.
