...
- 엔트리 2형식: `이름:역할:salt:해시`(번호 등록 — 로그인 가능) / `이름:역할`(**번호 미등록** — 명단에만 오르고 로그인 불가).
  이름을 먼저 등록하고 번호는 나중에 채우는 순서를 위한 것이다(2026-09-07 디렉터 지시).
  번호 미등록자는 로그인 화면에 `(번호 미등록)`으로 보이되 4자리 입력이 잠기고, 폼을 우회해 `/api/login`을
  직접 호출해도 401이다. 불변조건("번호 미등록으로 인증 성공하는 경로 없음")은 `scripts/roster-selftest.mjs`가 시험한다.
