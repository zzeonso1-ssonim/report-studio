...
- **웹앱(7)**: CPI 나우캐스트, GDP 나우캐스트, BOK 스탠스 랩, FOMC 소수의견 대시보드, 채권 모니터,
  DAAI(팀 공유·로그인 필요), 채권 판단 데스크 bond-desk(실시간 상태 표기)
- **저장소만(0)**: 비공개 저장소 링크는 제거됨(2026-08-03)

#### 모델 상태 표기 (`health` 선언 모델만)

레지스트리에 `health`를 선언한 모델은 카드에 **그 모델이 공개한 가벼운 상태 엔드포인트**의 값을 함께 보여준다.
조회·표기 로직은 `lib/model-status.ts`, 대상·필드·라벨은 전부 `lib/models.ts`의 선언에서 파생한다(URL 중복 표기 없음 — 호스트는 `url`에서 파생).

- **가벼운 것만** — bond-desk는 `/api/health`(수백 바이트)를 쓴다. 전체 run JSON(`?kind=run`, 1.5MB)은 목록 화면에서 부르지 않는다(모바일 고려)
- **기준일 병기 필수** — `generated_at`을 못 읽으면 수치를 내지 않고 실패로 표기한다
- **AI 결론은 초안 표기** — 사람 확정 플래그(`confirmed`)가 false면 "AI 초안 · 미확정" 뱃지를 단다
- **실패는 명시** — 제한시간 3.5초, 실패 시 "상태 불러오지 못함 + 사유"(HTTP 404 / 응답 없음 등). 조용한 빈칸을 두지 않고, 외부 앱이 죽어도 목록은 그대로 렌더된다(Suspense 밖에서 렌더 차단 없음)
- 결과는 5분 캐시(ISR) — 대상이 하루 1회 갱신이라 매 요청마다 부를 이유가 없다
